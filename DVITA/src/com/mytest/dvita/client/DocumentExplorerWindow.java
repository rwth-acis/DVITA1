...
import com.mytest.dvita.client.DVita.ClickHandlerShowDocument;
...
public class DocumentExplorerWindow extends Window {
	...
	public void setService(DocumentServiceAsync greetingService, DVita test) {
	...
	public void UpdateWindowByDocID(DocumentInfo infos, boolean deleteTree, Integer parentId){
		...
		if(deleteTree==true){
			...
			if(!treeNodes.containsKey(infos.docID)){
				...
				docEntry.setAttribute("date",DVita.formatDateTime(infos.docDate));
			...
			if( parentId!=null){
				...
				if(!treeNodes.containsKey(infos.docID)){
					...
					children.setAttribute("date",DVita.formatDateTime(infos.docDate));
		...
		titlelabel.setContents("<b>"+infos.docTitle +", "+DVita.formatDateTime(infos.docDate)+":</b>");
		//titlelabel.set
		...
		label.setContents("Similar documents between "+DVita.formatDateTime(topicInterval[(int)infos.intervalID][0])+" and "+DVita.formatDateTime(topicInterval[(int) infos.intervalID][1]));
	...
	class ClickHandlerForwardBackward implements com.smartgwt.client.widgets.events.ClickHandler{
		...
		public void onClick(com.smartgwt.client.widgets.events.ClickEvent event) {
			if(event.getSource()==addleftArrow){
				...
				label.setContents(DVita.formatDateTime(topicInterval[(int)addinfos.intervalID][0])+"<br>"+DVita.formatDateTime(topicInterval[(int) addinfos.intervalID][1]));
	...
	class CallBackSimilarDocs implements AsyncCallback< DocumentInfo []> {
		...
		public void onSuccess(DocumentInfo[] result) {
			...
			for(int i=0; i<result.length; i++) {
				...
				entry.setAttribute("date",DVita.formatDateTime(result[i].docDate));
	...
	class  ClickHandlerDocumentSearch implements FormItemClickHandler, KeyPressHandler {
		...
		public void generateDocumentTree(DocumentInfo[] result){
			...
			for(int i=0;i<result.length;i++){
				...
				docEntry.setAttribute("date",DVita.formatDateTime(result[i].docDate));
