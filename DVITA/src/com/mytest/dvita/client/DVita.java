...
import com.mytest.dvita.shared.DVitaParameters;
 ...
 */
public class DVita implements EntryPoint {
	 ...
	 */
	private final DocumentServiceAsync theDocumentService = GWT
			.create(DocumentService.class);

	private final TopicServiceAsync topicService = GWT
			.create(TopicService.class);

	private final WordServiceAsync wordService = GWT
	...
	//final  IButton back= new IButton();
	//////////////////////////////////////////////////////////////////////////////////////////////
	...
	public void onModuleLoad() {
		...
		TabSelectedHandler handler = new TabSelectedHandler() {
			...
			// wenn alles initialisiert wurde, dann zeiga das setup window
			SetupWindow s = new SetupWindow(thisWindow,true);

			String topicMinigID = com.google.gwt.user.client.Window.Location.getParameter("id");
			if(topicMinigID!= null) {
				int id = -1;
				try {
					id = Integer.parseInt(topicMinigID);
					s.setUpSession(id);
				} catch (NumberFormatException e) {
					s.show(); // datenabnk auswählen
				}
			} else {
				s.show(); // beim start muss datenabnk ausgewählt werden
			}
	...
	private  void initializeThemeRiver(){
		...
		// estmal das Chart erzeugen und ganz viele Optionen setzen 
		themeRiverChart
		...
		class ChangeHandlerWordEvolution implements ChangeHandler {
				...
				topicService. getTimeintervals(
						new AsyncCallback<String[][]>() {
							...
							public void onSuccess(String [][] topictime) {
								...
								//documentBrowserCanvas.removeMembers(documentBrowserCanvas.getMembers());
								//canvas_documentsOfTopic.clear();
								for(Record r : documentGrid.getRecords()) {
									documentGrid.removeData(r);
								}
								initializeThemeRiverForWords();
			...
			public static String formatTopicLabel(String[] words) {
				...
				for (int j=0; j<words.length; j++){
					if(words[j] == null || words[j].length() == 0)
						break; // less than 4 words in topic
			...
			public CheckBox[] generateTopicCheckboxes(TopicLabels result){
				...
				for(int i=0;i<result.topicIDs.length; i++){
					...
					/*for (int j=0; j<result.words[i].length; j++){
						//s= s+", "+String.valueOf(result.words[i][j]);
						if(j > 0) s += WORDSEPARATOR;
						String w = String.valueOf(result.words[i][j]);						
			...
			TopicOverviewPanel.addMember(forSelecting);*/
					...
					colorLabels[i].addClickHandler(handler);
			...
			class ClickHandlerWortCheckbox implements ClickHandler{
				...
				private void createYaxis(Double [] res,String s){
					...
					wordEvolutionChart.addSeries(theSeries);	
				...
				private void sendSelectedWordsToServer(Integer [] WordIdsCheck) {
					...
					if(WordIdsCheck.length>0) {
						loadingWindow.start("Loading Words<br>",1);
					}
				...
				public void treatResult(WordEvolutionData result){
					...
					for(int i=0;i <nrselectedWords;i++){//beim Wort 0
						Double [] WordsValue = new Double[result.intervalIDs.length];
						for (int j=0;j<result.intervalIDs.length;j++){//zur zeit To...Tx
				...
				public void treatResult(ThemeRiverData[]result){
					...
					for(int topic : selectedTopics){
						...
						for (int i=0;i<themeRiverDataForTopic[topic].wordsAtTime.length;i++ ){
							...
							/*String  s=themeRiverDataForTopic[topic].wordsAtTime[i][0];
							for(int j=1; j<themeRiverDataForTopic[topic].wordsAtTime[i].length;j++){//Wörter
								s= s + WORDSEPARATOR + String.valueOf(themeRiverDataForTopic[topic].wordsAtTime[i][j]) ;
			...
			class ClickHandlerTopicFromThemeRiver implements SeriesClickEventHandler, PointClickEventHandler{
				...
				public void updatePanel(DocumentInfo [] result){ 
					...
					//documentAccordeaonPanel.setHeight100();
					//documentAccordeaonPanel.markForRedraw();
			...
			void redrawPieChart() {
				...
				((com.google.gwt.user.client.Element)pie.getElement()).getStyle().setOverflow(com.google.gwt.dom.client.Style.Overflow.VISIBLE);
				((com.google.gwt.user.client.Element)pie.getElement().getParentNode()).getStyle().setOverflow(com.google.gwt.dom.client.Style.Overflow.VISIBLE);
				((com.google.gwt.user.client.Element)pie.getElement().getParentNode().getParentNode()).getStyle().setOverflow(com.google.gwt.dom.client.Style.Overflow.VISIBLE);
				((com.google.gwt.user.client.Element)pie.getElement().getParentNode().getParentNode().getParentNode()).getStyle().setOverflow(com.google.gwt.dom.client.Style.Overflow.VISIBLE);
				((com.google.gwt.user.client.Element)pie.getElement().getParentNode().getParentNode().getParentNode().getParentNode()).getStyle().setOverflow(com.google.gwt.dom.client.Style.Overflow.VISIBLE);
				((com.google.gwt.user.client.Element)pie.getElement().getParentNode().getParentNode().getParentNode().getParentNode().getParentNode()).getStyle().setOverflow(com.google.gwt.dom.client.Style.Overflow.VISIBLE);

				((com.google.gwt.user.client.Element)pie.getElement().getChild(0)).getStyle().setOverflow(com.google.gwt.dom.client.Style.Overflow.VISIBLE);
				...
				.setFormatter(new DataLabelsFormatter() {  
					public String format(DataLabelsData dataLabelsData) { 
						...
						.setShadow(true)  
								...
								.setFormatter(new ToolTipFormatter() {  
									public String format(ToolTipData toolTipData) {  
										// return "<b>" + toolTipData.getPointName().split(WORDSEPARATOR)[0]+"<br />"+toolTipData.getPointName().split(WORDSEPARATOR)[1]+", ..." + "</b>: " + ((int)(1000*toolTipData.getYAsDouble()))/10.0 + " %";
